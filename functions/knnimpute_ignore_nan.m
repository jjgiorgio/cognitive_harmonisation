function imputed = knnimpute_ignore_nan(data,K,varargin)
...
%   distances from the neighboring columns. K is a positive integer.
%
...
%
%   See also ISNAN, FITCKNN, NANMEAN, NANMEDIAN, PDIST.
...
if nargin < 2
    K = 1;
elseif ~isnumeric(K) || ~isscalar(K) || K ~= round(K) || K < 1 
    error(message('bioinfo:knnimpute:InvalidValueK'));
end
...
for count = 1:numel(rows)
    ...
    for nearest = 2:size(ndx,1)-K+1
        ...
        dataVals = data(rows(count),ndx(nearest:nearest+K+L-1,cols(count)));
          ScannNan=0;
        while sum(~isnan(dataVals))<K % Update from MATLAB function to ignore closest neighbours also with a nan in the missing value col
            ScannNan=ScannNan+1;
        dataVals = data(rows(count),ndx(nearest:nearest+K+L-1+ScannNan,cols(count)));    
        end
        
        
        if useWMean
            if ~userWeights
                weights = 1./dists(2:K+L+1+ScannNan,cols(count));
            else
                if L > 0
